        ...
        try {
            ...
            if (zaireSeq.size() != 0) {
                ...
                if (lowSeqZaire.size() != 0) {
                    Util.printSoftSet(true, "LOW", lowSeqZaire, request, response);
                }
                if (midSeqZaire.size() != 0) {
                    Util.printSoftSet(true, "MID", midSeqZaire, request, response);
                }
                if (highSeqZaire.size() != 0) {
                    Util.printSoftSet(true, "HIGH", highSeqZaire, request, response);
                }
            ...
            if (sudanSeq.size() != 0) {
                ...
                if (lowSeqSudan.size() != 0) {
                    Util.printSoftSet(true, "LOW", lowSeqSudan, request, response);
                }
                if (midSeqSudan.size() != 0) {
                    Util.printSoftSet(true, "MID", midSeqSudan, request, response);
                }
                if (highSeqSudan.size() != 0) {
                    Util.printSoftSet(true, "HIGH", highSeqSudan, request, response);
                }
                Util.printDiv(response);
            }
            if (restonSeq.size() != 0) {
                ...
                if (lowSeqReston.size() != 0) {
                    Util.printSoftSet(true, "LOW", lowSeqReston, request, response);
                }
                if (midSeqReston.size() != 0) {
                    Util.printSoftSet(true, "MID", midSeqReston, request, response);
                }
                if (highSeqReston.size() != 0) {
                    Util.printSoftSet(true, "HIGH", highSeqReston, request, response);
                }
                Util.printDiv(response);
            }
            if (taiSeq.size() != 0) {
                ...
                if (lowSeqTai.size() != 0) {
                    Util.printSoftSet(true, "LOW", lowSeqTai, request, response);
                }
                if (midSeqTai.size() != 0) {
                    Util.printSoftSet(true, "MID", midSeqTai, request, response);
                }
                if (highSeqTai.size() != 0) {
                    Util.printSoftSet(true, "HIGH", highSeqTai, request, response);
                }
                Util.printDiv(response);
            }
            if (bundibugyoSeq.size() != 0) {
                ...
                if (lowSeqBundi.size() != 0) {
                    Util.printSoftSet(true, "LOW", lowSeqBundi, request, response);
                }
                if (midSeqBundi.size() != 0) {
                    Util.printSoftSet(true, "MID", midSeqBundi, request, response);
                }
                if (highSeqBundi.size() != 0) {
                    Util.printSoftSet(true, "HIGH", highSeqBundi, request, response);
                }
                Util.printDiv(response);
            }
...
<center> <form action="doublySoftSet.jsp">
        <button type="submit" tite="Calculate Frequent Amino Acid" class="btn btn-primary">Calculate Frequent Sequences</button>
    </form>
</center>
</body>
</html>

...
<%@page import="psc.servlet.modal.DoublySoftModal"%>
            ...
            try {
                ...
                if (zaire.size() != 0) {
                    ...
                    if (lowSeqZaire.size() != 0) {
                        DoublySoftModal dsm = new DoublySoftModal("LOW_VALUE_RANGE", lowSeqZaire);
                        dsm.printResult(FILE_PATH, dsm.getResult(), response);

                    }
                    if (midSeqZaire.size() != 0) {
                        DoublySoftModal dsm = new DoublySoftModal("MID_VALUE_RANGE", midSeqZaire);
                        dsm.printResult(FILE_PATH, dsm.getResult(), response);

                    }
                    if (highSeqZaire.size() != 0) {
                        DoublySoftModal dsm = new DoublySoftModal("HIGH_VALUE_RANGE", highSeqZaire);
                        dsm.printResult(FILE_PATH, dsm.getResult(), response);
